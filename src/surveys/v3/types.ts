// src/surveys/v3/types.ts

export type SurveyTemplateVersion = 'v3';
...
export interface QuestionBase {
  ...
  // Доп.метаданные для методики зрелости
  level?: 1 | 2 | 3 | 4 | 5;      // Явный уровень зрелости
  categoryKey?: string;           // Группа внутри секции/уровня
  weight?: number;                // Вес критерия (по умолчанию 1)
  critical?: boolean;             // Критичный критерий (обязателен для зачёта уровня)

  // условная видимость (UI может скрывать/показывать)
  visibleIf?: Condition;

  // флаг риска из v2 — оставляем на случай совместимости
  isRisk?: boolean;
}
...
export type Question =
  ...
  | NumberQuestion
  | DateQuestion
  | TableQuestion; // поддержка таблиц
