...
class ApiService {
  ...
  constructor() {
    ...
    // Публичные эндпоинты без /api (см. исключения в main.ts)
    this.publicApi = axios.create({ baseURL: BASE });

    // Приватные перехватчики
    this.api.interceptors.request.use((config: InternalAxiosRequestConfig) => {
      const url = config.url ?? '';
      const path = url.startsWith('/') ? url : `/${url}`;

      const isPublic = path.startsWith('/public/') || path.startsWith('/survey/');
    ...
    // Публичные перехватчики (без авторизации)
    this.publicApi.interceptors.request.use((config: InternalAxiosRequestConfig) => {
      config.headers = config.headers ?? {};
      const isFormData =
        typeof FormData !== 'undefined' && config.data instanceof FormData;
      if (!isFormData && !(config.headers as any)['Content-Type']) {
      ...
      // Явно убираем Authorization на публичных
      delete (config.headers as any).Authorization;
      return config;
    });
  ...
  // =========================
  // PUBLIC (token-based, как в main.ts)
  // =========================
  ...
  // Открытие
  openSurveyByToken(token: string) {
    return this.publicPost<any>(`/survey/${encodeURIComponent(token)}/open`, {});
  }
  // Черновик
  getPublicSurveyDraftByToken(token: string) {
    return this.publicGet<any>(`/survey/${encodeURIComponent(token)}/draft`);
  }
  saveSurveyDraftByToken(token: string, payload: { answers: any; respondentMeta?: any }) {
    return this.publicPost<any>(`/survey/${encodeURIComponent(token)}/draft`, payload);
  }
  // Промежуточное сохранение
  saveSurveyByToken(token: string, payload: { answers: any; respondentMeta?: any }) {
    return this.publicPost<any>(`/survey/${encodeURIComponent(token)}/save`, payload);
  }
  // Отправка
  submitSurveyByToken(token: string, payload: { answers: any; respondentMeta?: any }) {
    return this.publicPost<any>(`/survey/${encodeURIComponent(token)}/submit`, payload);
  }
  // Результаты
  getSurveyResultsByToken(token: string) {
    return this.publicGet<any>(`/survey/${encodeURIComponent(token)}/results`);
  }

  // ---- Алиасы (token как публичный ключ анкеты)
  getSurveyLink(token: string) {
    return this.getPublicSurveyByToken(token);
  }
  openSurvey(token: string) {
    return this.openSurveyByToken(token);
  }
  submitSurveyResponse(token: string, payload: { answers: any; respondentMeta?: any }) {
    return this.submitSurveyByToken(token, payload);
  }
  getSurveyResults(token: string) {
    return this.getSurveyResultsByToken(token);
  }
}
...
export const openSurveyByToken = api.openSurveyByToken.bind(api);
export const getPublicSurveyDraftByToken = api.getPublicSurveyDraftByToken.bind(api);
export const saveSurveyDraftByToken = api.saveSurveyDraftByToken.bind(api);
export const saveSurveyByToken = api.saveSurveyByToken.bind(api);
export const submitSurveyByToken = api.submitSurveyByToken.bind(api);
export const getSurveyResultsByToken = api.getSurveyResultsByToken.bind(api);

// Aliases (используйте token)
